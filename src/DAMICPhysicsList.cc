...
void DAMICPhysicsList::ConstructEM() {
	...
	G4VPhysicsConstructor *penelopeEmPhysicsList = new G4EmPenelopePhysics();
	penelopeEmPhysicsList->ConstructProcess();
...
void DAMICPhysicsList::SetCuts()
{
	...
	// because some processes for e+/e- need cut values for gamma
	 ...
	 actcut->SetProductionCut(150*nm);
	 G4Region* region = G4RegionStore::GetInstance()->GetRegion("ActiveRegion");
	 region->SetProductionCuts(actcut);

	if (verboseLevel>0) DumpCutValuesTable();
}

//Code taken from http://hypernews.slac.stanford.edu/HyperNews/geant4/get/hadronprocess/1538/1/1.html (borrowed with love from CDMS, thx guys)

void DAMICPhysicsList::ConstructTritium() {
